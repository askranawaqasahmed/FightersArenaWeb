...
// The email address is the credential; a mobile number is optional contact detail.
...
export async function POST(request: Request) {
  try {
    ...
    if (input.phone) {
      try { phone = normalizePhone(input.phone); } catch { return apiProblem(422, "INVALID_PHONE", "Invalid phone number", "Use a valid mobile number, for example 03001234567."); }
    }
    const email = input.email.trim().toLowerCase();
    if (!checkRateLimit(`signup:${email}`, { limit: 5, windowMs: 15 * 60_000 })) {
    ...
    const result = await db.transaction(async (tx) => {
      ...
      if (phone) await tx.insert(userIdentities).values({ userId: user.id, type: "phone", normalizedValue: phone });
    ...
    const response = apiData({
      ...
      user: { id: result.userId, email, phone, accountType: "gamer", profile: result.profile },
    }, { status: 201 });
    return attachSessionCookies(response, accessToken, result.refreshToken);
  } catch (error) {
    if (error instanceof Error && "code" in error && (error as { code?: string }).code === "23505") {
      return apiProblem(409, "EMAIL_TAKEN", "Account already exists", "An account with this email address already exists. Sign in instead.");
    }
