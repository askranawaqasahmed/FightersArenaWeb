...
export function TournamentBuilder({ initialDraft, originalSlug, lifecycleCompetitions = [] }: TournamentBuilderProps) {
  ...
  function addCompetition() {
    ...
    const usedGames = new Set(draft.competitions.map((competition) => competition.gameSlug));
    const gameSlug = (["dota-2", "valorant", "tekken-8"] as const).find((game) => !usedGames.has(game)) ?? "dota-2";
    const gameName = gameSlug === "dota-2" ? "Dota 2" : gameSlug === "valorant" ? "VALORANT" : "Tekken 8";
    updateDraft({ competitions: [...draft.competitions, createGameCompetition(id, { name: `${gameName} competition`, gameSlug })] });
  }
  ...
  function validateDraft() {
    ...
    for (const competition of draft.competitions) {
      ...
      if (competition.stages.length < 1) return `${competition.name} must contain at least one stage.`;
    ...
    <main className={`admin-content${originalSlug ? " event-editor-full" : ""}`}>
      ...
      <section className="event-builder-full">
          ...
          <div className="activity">
            {draft.competitions.map((competition, index) => <GameCompetitionEditor key={competition.id} competition={competition} index={index} canRemove={draft.competitions.length > 1} onChange={(updated) => updateCompetition(competition.id, updated)} onRemove={() => removeCompetition(competition.id)} />)}
          </div>
      </section>
